...
class Chamber_Button:
    def __init__(self,master,port_button,button_place):
        self.port_button = port_button
        self.master = master
        self.button = Button(button_place,text='Chamber',compound=TOP,command=self.click)
        ...
        self.chamber_created = True
        self.delete_last_button = Button(button_place,text='undo',command=self.undo)
        self.delete_last_button.grid(row=1,column=3)


    def click(self):
        global cha
        if self.chamber_created:
            cha     = Chamber(self.master.master,self,self.port_button)
            cha.num = self.count
            cha.click_once_bind = cha.master.bind('<1>',cha.click_once,'+')

            self.chambers[cha.chamber] = cha
...
class Chamber_Display:
    def __init__(self,master,chamber):
        self.chamber = chamber
        self.master  = master
        self.window  = master
        self.window.title('Chamber Information')
        self.window.geometry('250x130-10000-100000')
        
        self.window.grid_columnconfigure(0,weight=3)
        self.window.grid_columnconfigure(1,weight=1)
        self.font   = 'Helvetica 10'
        self.labels = [' Chamber Shape : ',
                       ' Chamber Depth (mm): ',
                       ' Chamber Diameter (mm): ',
                       ' Flow Rate (\u03BCL/s) : ',
                       ]
...
class Chamber_Edit:
    def __init__(self,master,chamber):
        ...
        self.master  = master
        self.window  = master
        self.window.geometry(f'550x100-10000-100000')
        ...
        self.title       =  Label(self.window,text='Edit Chamber Information',font='Arial 10 bold underline',width=30,anchor=W)
    ...
    def place(self):
        ...
        for i in range(4,5): # range(4,7) for row 4-6  (labels @ row 3)
            ...
            self.selected_shape.set(shapes[i-4])
            self.dropdown_shapes = OptionMenu(self.window,self.selected_shape,*shapes)
            self.dropdown_shapes.grid(row=i,column=0)

            self.depth = Entry(self.window,width=10,justify='center')
            self.depth.insert(0,f'{5*(i-3)}')
            self.depth.grid(row=i,column=1)

            self.diameter = Entry(self.window,width=10,justify='center')
            self.diameter.insert(0,f'{2+i-4}')
            self.diameter.grid(row=i,column=2)

            self.flow_rate = Entry(self.window,width=10,justify='center')
            self.flow_rate.insert(0,'')
            self.flow_rate.grid(row=i,column=3)

            self.data = [self.selected_shape,self.depth,self.diameter,self.flow_rate]
    ...
    def save_changes(self):
        ...
        self.chamber.data = self.values.copy()
        self.chamber.display.shape     = self.values[0]
        self.chamber.display.depth     = self.values[1]
        self.chamber.display.diameter  = self.values[2]
        self.chamber.display.flow_rate = self.values[3]
        self.chamber.display.update()
...
class Chamber:
    def __init__(self,master,chamber_button,port_button):
        self.port_button  = port_button
        self.chamber_button  = chamber_button
        self.master          = master
        self.edit_win        = Toplevel(self.master)
        self.disp_win        = Toplevel(self.master)
        self.display         = Chamber_Display(self.disp_win,self)
        self.edit            = Chamber_Edit(   self.edit_win,self)
        ...
        self.chamber         = self.master.create_line(0,0,0,0,dash=(10,2),width=2,tags='chamber',fill='light green')
        self.label           = Label(self.master,text='Click on 2 ports to create chamber',bg='dark grey')
        self.data            = []
    ...
    def bind(self):
        ...
        self.master.tag_bind(self.chamber,'<Double-1>',partial(self.show_window,self.edit_win))
        self.master.tag_bind(self.chamber,'<Enter>'   ,partial(self.show_window,self.disp_win))
        self.master.tag_bind(self.chamber,'<Leave>'   ,partial(self.hide_window,self.disp_win))

    def moving(self,event):
        self.line_data['x2'] = event.x
        self.line_data['y2'] = event.y

        self.master.coords(self.chamber,self.line_data['x1'], self.line_data['y1'],self.line_data['x2'], self.line_data['y2'])
    ...
    def dragged(self):
        ...
        self.master.coords(self.chamber,self.line_data['x1'], self.line_data['y1'],
    ...
    def click_once(self,event):
        self.chamber_button.chamber_created = False
        r = 20
        x = event.x
        y = event.y
        ...
        if len(n_overlap) > 1:
            for obj in n_overlap:
                if self.master.type(obj) == 'oval' and obj != self.front:
                    ...
                    self.master.itemconfig(self.chamber,dash=())
                    self.chamber_button.chamber_created = True
    ...
    def destroy(self,event):
        self.cancel = True
        self.master.delete(self.chamber)
